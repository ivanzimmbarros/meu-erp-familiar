...
# --- INSERIR ESTA FUNÇÃO ABAIXO DE init_db() ---
def patch_db_security_v5():
    """Garante a coluna de controle de troca de senha."""
    try:
        db_execute("ALTER TABLE usuarios ADD COLUMN force_reset INTEGER DEFAULT 0")
    except:
        pass # Coluna já existe

# --- INSERIR ESTAS DUAS LINHAS LOGO ABAIXO DA DEFINIÇÃO DA FUNÇÃO ACIMA ---
init_db()
...
def liquidar_transacao(trans_id, tipo, usuario):
    """Restaura a funcionalidade de dar baixa em pagamentos/recebimentos."""
...
try:
    if st.secrets.get("emergency", {}).get("create_admin"):
        ...
        with sqlite3.connect(DB_PATH) as conn:
            conn.execute("INSERT OR IGNORE INTO usuarios (username, password, nome_exibicao, email, perfil, force_reset) VALUES (?,?,?,?,?,?)",
                ("admin", pwd_h, "Administrador", "admin@local.dev", "Administrador", 0))
            conn.execute("UPDATE usuarios SET password=?, perfil='Administrador', force_reset=0 WHERE username='admin'", (pwd_h,))
            conn.commit()
except Exception:
    pass
